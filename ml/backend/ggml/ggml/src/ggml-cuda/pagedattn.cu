...
static void ggml_cuda_paged_attention_f16(ggml_backend_cuda_context & ctx, ggml_tensor * dst);
...
// Vectorized FP16 dot product
template<int N>
__device__ __forceinline__ float dot_product_fp16(const half* a, const half* b) {
    float sum = 0.0f;
    #pragma unroll
    for (int i = 0; i < N; i++) {
...
// FP16 PagedAttention Kernel - Mixed Precision
// Uses FP16 for Q/K/V storage and computation, FP32 for softmax accumulation
template <int HEAD_DIM, int BLOCK_SIZE>
static __global__ void paged_attention_kernel_f16(
    ...
    // Load query vector into shared memory (use FP32 for computation)
    __shared__ float q_vec_f32[HEAD_DIM];
    const half * q_src = (const half *)((const char *)Q + head_idx * nbq2 + seq_idx * nbq3);

    // Convert Q to FP32 for computation
    for (int i = threadIdx.x; i < HEAD_DIM; i += blockDim.x) {
        q_vec_f32[i] = __half2float(q_src[i]);
    }
    __syncthreads();

    // Online softmax state and output accumulator (per thread, FP32)
    constexpr int N_ELEMS = (HEAD_DIM + PAGED_ATTN_THREADS - 1) / PAGED_ATTN_THREADS;
    int elem_start = threadIdx.x * N_ELEMS;
    int elem_end   = min(elem_start + N_ELEMS, HEAD_DIM);

    float out_acc_f32[N_ELEMS];
    float max_logit = -FLT_MAX;
    float sum_exp   = 0.0f;
    ...
    // Shared memory for block-level scores (FP32 for softmax)
    __shared__ float block_scores[BLOCK_SIZE];
    __shared__ float block_max;
    __shared__ float block_sum_exp;
    ...
    for (int block_idx = 0; block_idx < num_blocks; block_idx++) {
        ...
        // Compute attention scores: Q @ K (FP32 computation)
        for (int pos = threadIdx.x; pos < block_len; pos += blockDim.x) {
            float score = 0.0f;
            const char * k_ptr = (const char *)K_cache +
                pos * nbk2 + kv_head_idx * nbk1 + physical_block * nbk3;
            const half * k_data = (const half *)k_ptr;

            // Dot product: Q (FP32) @ K (FP16)
            for (int d = 0; d < HEAD_DIM; d++) {
                score += q_vec_f32[d] * __half2float(k_data[d]);
            }
            block_scores[pos] = score * scale;
        ...
        #pragma unroll
        for (int i = 0; i < N_ELEMS; i++) {
            out_acc_f32[i] *= correction;
        }

        // Accumulate weighted values (V is FP16)
        for (int pos = threadIdx.x; pos < block_len; pos += blockDim.x) {
            float weight = expf(block_scores[pos] - m_new);

            const char * v_ptr = (const char *)V_cache +
                pos * nbv2 + kv_head_idx * nbv1 + physical_block * nbv3;
            const half * v_data = (const half *)v_ptr;

            for (int i = 0; i < N_ELEMS; i++) {
                int d = elem_start + i;
                if (d < HEAD_DIM) {
                    out_acc_f32[i] += weight * __half2float(v_data[d]);
                }
            }
        }
        __syncthreads();
    ...
    // Normalize output (FP32 -> FP16)
    float inv_sum = 1.0f / sum_exp;

    half * dst_row = (half *)((char *)dst + head_idx * nbq2 + seq_idx * nbq3);
    for (int i = 0; i < N_ELEMS; i++) {
        int d = elem_start + i;
        if (d < HEAD_DIM) {
            dst_row[d] = __float2half(out_acc_f32[i] * inv_sum);
        }
    }
}
...
bool ggml_cuda_paged_attention_supported(const ggml_tensor * dst) {
    ...
    const bool all_f32 = (q->type == GGML_TYPE_F32 &&
                          k->type == GGML_TYPE_F32 &&
                          v->type == GGML_TYPE_F32);
    const bool all_f16 = (q->type == GGML_TYPE_F16 &&
                          k->type == GGML_TYPE_F16 &&
                          v->type == GGML_TYPE_F16);

    return all_f32 || all_f16;
}
...
void ggml_cuda_op_paged_attention(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (q->type == GGML_TYPE_F16) {
        ggml_cuda_paged_attention_f16(ctx, dst);
    } else {
        ggml_cuda_paged_attention_f32(ctx, dst);
    }
}

// FP16 dispatch function
static void ggml_cuda_paged_attention_f16(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ggml_tensor * q            = dst->src[0];
    ggml_tensor * k_cache      = dst->src[1];
    ggml_tensor * v_cache      = dst->src[2];
    ...
    const half * k_data    = (const half *)k_cache->data;
    const half * v_data    = (const half *)v_cache->data;
    const float * mask_data = mask ? (const float *)mask->data : nullptr;
    const int32_t * bt_data = (const int32_t *)block_tables->data;
    const int32_t * sl_data = (const int32_t *)seq_lengths->data;
    half * dst_data        = (half *)dst->data;
...
#define CASE_HEAD_DIM(D)                                                           \
        case D:                                                                    \
            switch (block_size) {                                                  \
                case 8:                                                            \
                    paged_attention_kernel_f16<D, 8><<<grid, block, 0, ctx.stream()>>>( \
                        q_data, k_data, v_data, mask_data, bt_data, sl_data,       \
                        dst_data, nbq2, nbq3, nbk1, nbk2, nbk3, nbv1, nbv2, nbv3, \
                        max_blocks_seq, gqa_ratio, scale);                         \
                    break;                                                         \
                case 16:                                                           \
                    paged_attention_kernel_f16<D, 16><<<grid, block, 0, ctx.stream()>>>( \
                        q_data, k_data, v_data, mask_data, bt_data, sl_data,       \
                        dst_data, nbq2, nbq3, nbk1, nbk2, nbk3, nbv1, nbv2, nbv3, \
                        max_blocks_seq, gqa_ratio, scale);                         \
                    break;                                                         \
                case 32:                                                           \
                    paged_attention_kernel_f16<D, 32><<<grid, block, 0, ctx.stream()>>>( \
                        q_data, k_data, v_data, mask_data, bt_data, sl_data,       \
                        dst_data, nbq2, nbq3, nbk1, nbk2, nbk3, nbv1, nbv2, nbv3, \
                        max_blocks_seq, gqa_ratio, scale);                         \
                    break;                                                         \
...
#undef CASE_HEAD_DIM
        ...
        default:
            // Fallback: round up to nearest supported
            if (head_dim <= 80) {
                paged_attention_kernel_f16<80, 16><<<grid, block, 0, ctx.stream()>>>(
                    q_data, k_data, v_data, mask_data, bt_data, sl_data,
                    dst_data, nbq2, nbq3, nbk1, nbk2, nbv3, nbv1, nbv2, nbv3,
                    max_blocks_seq, gqa_ratio, scale);
            } else if (head_dim <= 128) {
                paged_attention_kernel_f16<128, 16><<<grid, block, 0, ctx.stream()>>>(
                    q_data, k_data, v_data, mask_data, bt_data, sl_data,
                    dst_data, nbq2, nbq3, nbk1, nbk2, nbk3, nbv1, nbv2, nbv3,
                    max_blocks_seq, gqa_ratio, scale);
            } else if (head_dim <= 256) {
                paged_attention_kernel_f16<256, 16><<<grid, block, 0, ctx.stream()>>>(
                    q_data, k_data, v_data, mask_data, bt_data, sl_data,
                    dst_data, nbq2, nbq3, nbk1, nbk2, nbk3, nbv1, nbv2, nbv3,
                    max_blocks_seq, gqa_ratio, scale);
            } else {
                paged_attention_kernel_f16<512, 16><<<grid, block, 0, ctx.stream()>>>(
                    q_data, k_data, v_data, mask_data, bt_data, sl_data,
                    dst_data, nbq2, nbq3, nbk1, nbk2, nbk3, nbv1, nbv2, nbv3,
                    max_blocks_seq, gqa_ratio, scale);
            }
